...
import { FaTrophy, FaMedal, FaCrown, FaFire, FaLeaf, FaArrowUp, FaStar, FaAward } from 'react-icons/fa';
...
const LeaderboardWidget = () => {
  ...
  const leaderboardData = [
    ...
    {
      rank: 4,
      name: user?.name || 'Arjun Sharma (You)',
      avatar: user?.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'Arjun')}&background=10b981&color=fff`,
      points: user?.points || 470,
      recycledKg: 28.5,
      tier: 'Green Warrior',
      badge: '🎖️ Rising Star',
      isCurrentUser: true
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-6 rounded-3xl shadow-sm space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="font-black text-slate-900 dark:text-white text-base flex items-center space-x-2">
              <span>Community Eco Leaderboard</span>
              <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 text-[9px] uppercase tracking-wider font-extrabold border border-emerald-500/20">
                Top Recyclers
              </span>
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Climb ranks by recycling waste & earning EcoPoints</p>
          </div>
        </div>
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl text-xs font-extrabold">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl transition-all ${filter === 'monthly' ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            This Month
          </button>
          <button
            onClick={() => setFilter('allTime')}
            className={`px-3.5 py-1.5 rounded-xl transition-all ${filter === 'allTime' ? 'bg-emerald-600 text-white shadow' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            All Time
          </button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 text-center pt-2">
        ...
        <div className="p-3 bg-gradient-to-b from-slate-100/80 to-slate-200/50 dark:from-slate-800/80 dark:to-slate-850/50 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 flex flex-col items-center space-y-2 relative">
          ...
          <img src={leaderboardData[1].avatar} alt="2nd" className="h-12 w-12 rounded-full object-cover ring-2 ring-slate-300 mt-2" />
          <div>
            <p className="font-extrabold text-slate-900 dark:text-white text-xs truncate max-w-[90px]">{leaderboardData[1].name}</p>
            <span className="text-[10px] font-black text-emerald-500">{leaderboardData[1].points} Pts</span>
          </div>
        </div>
        ...
        <div className="p-3 bg-gradient-to-b from-amber-500/20 via-yellow-500/10 to-amber-500/5 rounded-2xl border-2 border-amber-400/60 flex flex-col items-center space-y-2 relative shadow-lg transform -translate-y-2">
          ...
          <img src={leaderboardData[0].avatar} alt="1st" className="h-14 w-14 rounded-full object-cover ring-4 ring-amber-400 mt-2 shadow-md" />
          <div>
            <p className="font-black text-slate-900 dark:text-white text-xs truncate max-w-[100px]">{leaderboardData[0].name}</p>
            <span className="text-xs font-black text-amber-500">{leaderboardData[0].points} Pts</span>
          </div>
        </div>
        ...
        <div className="p-3 bg-gradient-to-b from-amber-900/10 to-amber-800/5 dark:from-amber-950/40 dark:to-amber-900/20 rounded-2xl border border-amber-700/40 flex flex-col items-center space-y-2 relative">
          ...
          <img src={leaderboardData[2].avatar} alt="3rd" className="h-12 w-12 rounded-full object-cover ring-2 ring-amber-700 mt-2" />
          <div>
            <p className="font-extrabold text-slate-900 dark:text-white text-xs truncate max-w-[90px]">{leaderboardData[2].name}</p>
            <span className="text-[10px] font-black text-emerald-500">{leaderboardData[2].points} Pts</span>
          </div>
        </div>
      </div>
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400 font-bold block">{item.badge} • {item.recycledKg} kg Recycled</span>
              </div>
            </div>
